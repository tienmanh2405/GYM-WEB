...
class HomeController_KhuyenMai {
    ...
    public function index() {
        ...
        require_once BASE_PATH . '/views/Admin/quanlykhuyenmai/QuanLyKhuyenMai.php';
    }
    ...
    public function create() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            header('Location: ' . BASE_URL . 'Admin/quanlykhuyenmai');
            exit();
        }
    ...
    public function update() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $ngayKetThuc = $_POST['ngayKetThuc'];
            $moTa = $_POST['moTa'];
            $maGoiTap = $_POST['maGoiTap']; // Thêm mã gói tập

            $this->khuyenMaiModel->updateKhuyenMai($maKhuyenMai, $giamGia, $ngayBatDau, $ngayKetThuc, $moTa, $maGoiTap);
            header('Location: ' . BASE_URL . 'Admin/quanlykhuyenmai');
            exit();
        }

        require_once BASE_PATH . '/views/Admin/quanlykhuyenmai/QuanLyKhuyenMai.php'; // Đảm bảo gọi view ở đây
    }

    // Xóa khuyến mãi theo mã
    public function delete($maKhuyenMai) {
        if ($this->khuyenMaiModel->deleteKhuyenMai($maKhuyenMai)) {
            header('Location: ' . BASE_URL . 'Admin/quanlykhuyenmai?message=delete_success');
        } else {
            header('Location: ' . BASE_URL . 'Admin/quanlykhuyenmai?message=delete_failed');
        }
        exit();
    }
}
