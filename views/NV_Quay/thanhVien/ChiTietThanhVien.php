...
<body>
    ...
    <?php 
        ...
        if ($userID) {
            // Lấy thông tin gói đăng ký của thành viên theo userID
            $goiDangKyList = $thanhVienModel->getGoiDangKyByUserID($userID);
            foreach ($goiDangKyList as $item) {
                echo "ID Gói Đăng Ký: " . $item['idDangKy'] . "<br>";
                echo "Mã Gói Tập: " . $item['maGoiTap'] . "<br>";
                echo "Ngày Hết Hạn: " . $item['ngayHetHan'] . "<br>";
                echo "Trạng Thái: " . $item['trangThai'] . "<br>";
                echo "Ngày Mua: " . $item['ngayMua'] . "<br><br>";
            }
        }else {
            echo "<p>Không có thông tin gói đăng ký cho thành viên này.</p>";
        }
